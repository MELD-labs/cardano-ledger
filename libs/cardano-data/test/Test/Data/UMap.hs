...
data Coin = Coin Integer
  deriving (Eq, Ord)

instance Show Coin where show (Coin n) = "(Coin " ++ show n ++ ")"

instance Semigroup Coin where (Coin x) <> (Coin y) = Coin (x + y)

instance Monoid Coin where mempty = Coin 0

instance (Show coin, Show cred, Show pool, Show ptr) => Show (VMap coin cred pool ptr x y) where
...
type View = VMap Coin Cred Pool Ptr
...
genPool :: Gen Pool
genPool = oneof (map pure "abcdefghijklmnopqrstuvwxyz")
...
loop :: Ord k => Maybe (k, v, VMap coin cr pl ptr k v) -> Map k v -> Map k v
...
finish ::
  Ord cred =>
  Maybe (a, b, VMap coin cred pool ptr k v) ->
  Maybe (a, b, Map k v)
