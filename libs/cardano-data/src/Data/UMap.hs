...
data Trip coin pool ptr = Triple
...
viewTrip :: Trip coin pool ptr -> (StrictMaybe coin, Set ptr, StrictMaybe pool)
...
pattern Triple :: StrictMaybe coin -> Set ptr -> StrictMaybe pool -> Trip coin pool ptr
...
instance (Show coin, Show pool, Show ptr) => Show (Trip coin pool ptr) where
  show (Triple a b c) = "(Triple " ++ show a ++ " " ++ show b ++ " " ++ show c ++ ")"

-- =====================================================

data UMap coin cred pool ptr = UnifiedMap (Map cred (Trip coin pool ptr)) (Map ptr cred)
  deriving (Show, Eq, Generic, NoThunks, NFData)
...
data VMap coin cr pl ptr k v where
  Rewards ::
    UMap coin cr pl ptr ->
    VMap coin cr pl ptr cr coin
  Delegations ::
    UMap coin cr pl ptr ->
    VMap coin cr pl ptr cr pl
  Ptrs ::
    UMap coin cr pl ptr ->
    VMap coin cr pl ptr ptr cr

-- ==================================================
-- short hand constructors and selectors

rewards ::
  Map cr (Trip coin pool ptr) ->
  Map ptr cr ->
  VMap coin cr pool ptr cr coin
rewards x y = Rewards (UnifiedMap x y)

delegations ::
  Map cred (Trip coin pool ptr) ->
  Map ptr cred ->
  VMap coin cred pool ptr cred pool
delegations x y = Delegations (UnifiedMap x y)

ptrs ::
  Map cred (Trip coin pool ptr) ->
  Map ptr cred ->
  VMap coin cred pool ptr ptr cred
ptrs x y = Ptrs (UnifiedMap x y)

unView :: VMap coin cr pl ptr k v -> UMap coin cr pl ptr
...
unUnify :: Ord cred => VMap coin cred pool ptr k v -> Map k v
...
instance (Ord pool, Monoid coin) => Semigroup (Trip coin ptr pool) where
  (<>) (Triple c1 ptrs1 x) (Triple c2 ptrs2 y) = Triple (addC c1 c2) (Set.union ptrs1 ptrs2) (add x y)
    where
...
addC :: Monoid x => StrictMaybe x -> StrictMaybe x -> StrictMaybe x
addC SNothing SNothing = SNothing
addC (SJust w) SNothing = SJust w
addC SNothing (SJust z) = SJust z
addC (SJust c1) (SJust c2) = SJust (c1 <> c2)

instance (Ord pool, Monoid coin) => Monoid (Trip coin ptr pool) where
...
zero :: Trip coin pool ptr -> Bool
zero (Triple SNothing s SNothing) | Set.null s = True
zero _ = False

zeroMaybe :: Trip coin pool ptr -> Maybe (Trip coin pool ptr)
...
next :: VMap coin cr pl ptr k v -> Maybe (k, v, VMap coin cr pl ptr k v)
...
leastUpperBound :: (Ord ptr, Ord cr) => k -> VMap coin cr pool ptr k v -> Maybe (k, v, VMap coin cr pool ptr k v)
...
delete :: (Ord cr, Ord ptr) => k -> VMap coin cr pool ptr k v -> UMap coin cr pool ptr
...
lookup :: (Ord cr, Ord ptr) => k -> VMap coin cr pool ptr k v -> Maybe v
...
isNull :: VMap coin cr pool ptr k v -> Bool
...
domain :: (Ord cr) => VMap coin cr pool ptr k v -> Set k
...
range :: (Ord coin, Ord pool, Ord cr) => VMap coin cr pool ptr k v -> Set v
...
(∪+) ::
  ...
  VMap coin cred pool ptr k coin ->
  Map k coin ->
...
unionHelp ::
  (Ord k, Monoid coin) =>
  Map k (Trip coin pool ptr) ->
  Map k coin ->
  Map k (Trip coin pool ptr)
unionHelp tm mm =
  Map.mergeWithKey
    (\_k (Triple c1 s d) c2 -> Just (Triple (addC c1 (SJust c2)) s d))
    id
...
member :: (Ord cr, Ord ptr) => k -> VMap coin cr pool ptr k v -> Bool
...
notMember :: (Ord cr, Ord ptr) => k -> VMap coin cr pool ptr k v -> Bool
...
domRestrict :: (Ord cr, Ord ptr) => VMap coin cr pool ptr k v -> Map k u -> Map k u
...
instance
  ...
  FromSharedCBOR (Trip coin pool ptr)
  where
  type Share (Trip coin pool ptr) = Interns pool
  fromSharedCBOR is =
...
-- One can do this using fromSharedPlusCBOR and Lenses and lift, but I think this is much clearer
-- using fromSharedPlusCBOR'. By giving a name to the shared state, i.e. (a,b) we can use
-- individual pieces of it where we need, without involving Lense combinators or lift. The only
-- drawback is we that we must build the bigger state  (internsFromMap tripmap <> a, b) by hand.
...
instance
  ...
  fromSharedPlusCBOR' (a, b) =
    decodeRecordNamed "UnifiedMap" (const 2) $ do
      ...
      pure (UnifiedMap tripmap ptrmap, (internsFromMap tripmap <> a, b))
...
class View coin cred pool ptr k v where
  tag :: Tag coin cred pool ptr k v

-- ===================================================
-- derived operations

findWithDefault :: (Ord cred, Ord ptr) => a -> k -> VMap coin cred pool ptr k a -> a
findWithDefault a1 k vm =
  case lookup k vm of
    Just a2 -> a2
    Nothing -> a1

size :: VMap coin cred pool ptr k a -> Int
