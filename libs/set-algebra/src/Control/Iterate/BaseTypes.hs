...
data BaseRep f k v where
  ...
  ViewR ::
    ...
    BaseRep (UM.VMap coin cred pool ptr) k v
...
instance
  ...
  Basic (UM.VMap coin cred pool ptr)
  where
...
instance
  ...
  Iter (UM.VMap coin cred pool ptr)
  where
  nxt m =
    Collect
      ( \ans f ->
...
instance Embed (UM.VMap coin cred pool ptr k v) (UM.VMap coin cred pool ptr k v) where
  toBase xs = xs
  fromBase xs = xs
