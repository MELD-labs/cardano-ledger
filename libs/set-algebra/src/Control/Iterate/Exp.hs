...
instance
  (View coin cred pool ptr k v, Ord k, Monoid coin, Ord coin, Ord cred, Ord ptr, Ord pool) =>
  HasExp
...
instance
  (View coin cred pool ptr k v, Monoid coin, Ord k, Ord coin, Ord cred, Ord ptr, Ord pool) =>
  HasQuery (VMap coin cred pool ptr k v) k v
  where
